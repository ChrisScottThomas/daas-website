...
const findAirtableRecordId = async (email) => {
...
const upsertToAirtable = async ({ email, foundingMember, source, emailSent, retryCount }) => {
  const token = await getAirtableToken();
  const recordId = await findAirtableRecordId(email);
...
exports.handler = async (event) => {
  try {
    ...
    if (existing.Item?.retryCount >= 3) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Retry limit reached' }),
      };
    }

    if (existing.Item?.emailDisabled) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Email disabled' }),
      };
    }
    ...
    const retryCount = (existing.Item?.retryCount || 0) + (emailSent ? 0 : 1);
    ...
    await upsertToAirtable({ email, foundingMember, source: utmSource, emailSent, retryCount });
